...
export default function InterestCard({ interest, headingLevel = 3 }) {
  ...
  return (
    <Card as="li" className="flex gap-4 p-5">
      <span
        className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-border bg-surface-elevated text-accent"
        aria-hidden="true"
      >
      ...
      <div className="min-w-0">
        ...
        {interest.note && <p className="mt-1 text-sm text-foreground-muted">{interest.note}</p>}
      </div>
    </Card>
  )
}
