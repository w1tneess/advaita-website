...
function collectErrors(document) {
  ...
  if (document.profile)
    for (const [key, message] of Object.entries(validateProfile(document.profile)))
      errors.push(`profile.${key}: ${message}`)
  if (document.settings)
    for (const [key, message] of Object.entries(validateSettings(document.settings)))
      errors.push(`settings.${key}: ${message}`)

  for (const [key, items] of Object.entries(document.categories || {})) {
    for (const [index, item] of items.entries())
      for (const [field, message] of Object.entries(validateCategory(item, items)))
        errors.push(`categories.${key}[${index}].${field}: ${message}`)
  }
  for (const [key, items] of Object.entries(validators)) {
    for (const [index, item] of (document[key] || []).entries()) {
      const siblings = key === 'projects' ? projectSlugs : document[key]
      for (const [field, message] of Object.entries(items(item, siblings)))
        errors.push(`${key}[${index}].${field}: ${message}`)
    }
  }
  return errors
}
