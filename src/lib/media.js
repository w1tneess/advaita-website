...
export function safeAssetFilename(name, type = '') {
  ...
  const stem = String(name).replace(/\.[^.]+$/, '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 80)
  return `${stem || 'image'}.${IMAGE_TYPES.has(type) ? IMAGE_TYPES.get(type) : extension || 'bin'}`
}

export function validateImageFile(file) {
  if (!file || !IMAGE_TYPES.has(file.type)) return { ok: false, error: 'Choose a PNG, JPEG, WebP, GIF, or AVIF image.' }
